...
import { Article, Button, Footer, Header, Input, Label, Main, Section, Select, Table, TableBody, TableDataCell, TableHead, TableHeaderCell, TableRow, Textarea } from '../../components/MaterialPrimitives/MaterialPrimitives'
    ...
    <div className="account-overlay" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <Section className="account-details" role="dialog" aria-modal="true">
        ...
        <div className="account-detail-body">
          ...
          <div className="detail-table-scroll"><Table><TableHead><TableRow><TableHeaderCell>Ngày</TableHeaderCell><TableHeaderCell>Loại</TableHeaderCell><TableHeaderCell>Danh mục</TableHeaderCell><TableHeaderCell>Số tiền</TableHeaderCell><TableHeaderCell>Đã thanh toán</TableHeaderCell><TableHeaderCell>Chưa thanh toán</TableHeaderCell><TableHeaderCell>Đối ứng</TableHeaderCell><TableHeaderCell>Thao tác</TableHeaderCell></TableRow></TableHead>
            <TableBody>{rows.map((row) => <TableRow key={row.id}><TableDataCell>{row.date}</TableDataCell><TableDataCell>{row.type}</TableDataCell><TableDataCell>{row.category}</TableDataCell><TableDataCell className={row.type === 'Thu' ? 'account-in' : 'account-out'}>{row.type === 'Thu' ? '+' : '-'} {money(row.amount)}</TableDataCell><TableDataCell>{money(row.paid)}</TableDataCell><TableDataCell>{money(Math.max(0, row.amount - row.paid))}</TableDataCell><TableDataCell>{row.counterparty}</TableDataCell><TableDataCell><div className="account-detail-actions"><ActionIcon icon="print" tone="green" label="In giao dịch" onClick={() => printRow(row)} /><ActionIcon icon="edit" label="Sửa giao dịch" onClick={() => setEditingRow(row)} /><ActionIcon icon="trash" tone="red" label="Xóa giao dịch" onClick={() => setPendingDelete(row)} /></div></TableDataCell></TableRow>)}</TableBody>
          </Table></div>
        </div>
